...
var searchreplace = (function () {
  ...
  var $_dtlj56isjdxo72fa = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_dtlj56isjdxo72fa.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_7hu68biqjdxo72ej.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_7hu68biqjdxo72ej.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_7hu68biqjdxo72ej.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_7hu68biqjdxo72ej.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_7hu68biqjdxo72ej.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_7hu68biqjdxo72ej.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_7hu68biqjdxo72ej.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_7hu68biqjdxo72ej.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_7hu68biqjdxo72ej.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_7hu68biqjdxo72ej.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_7hu68biqjdxo72ej.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_7hu68biqjdxo72ej.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_7hu68biqjdxo72ej.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_7hu68biqjdxo72ej.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_7hu68biqjdxo72ej.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_apvotuiujdxo72g9 = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_apvotuiujdxo72g9.open(editor, currentIndexState);
    });
  };
  var $_aydp8oitjdxo72g4 = { register: register };
  ...
  var $_dn541nivjdxo72gg = { register: register$1 };

  PluginManager.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_aydp8oitjdxo72g4.register(editor, currentIndexState);
    $_dn541nivjdxo72gg.register(editor, currentIndexState);
    return $_gdhx2hipjdxo72ef.get(editor, currentIndexState);
  });
  function Plugin () {
  }
